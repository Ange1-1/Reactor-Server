...
public:
    EchoServer(std::string_view ip,const uint16_t port,int subthreadnum=3,int workthreadnum=5);
    ~EchoServer();
    ...
    void HandleMessage(spConnection conn,std::string_view message);     // 处理客户端的请求报文，在TcpServer类中回调此函数。
    ...
    void OnMessage(spConnection conn,std::string_view message);     // 处理客户端的请求报文，用于添加给线程池。
};